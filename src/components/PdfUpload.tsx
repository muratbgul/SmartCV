  ...
  const onDrop = useCallback(async (acceptedFiles: File[]) => {
    ...
    try {
      ...
      try {
        ...
        setAnalysisSource(aiData?.source || 'unknown');
        
        if (aiData?.error) {
          setAnalysisError(aiData.error);
        } else {
        ...
        if (aiData?.debug) {
          try {
            const safeDebug = JSON.parse(JSON.stringify(aiData.debug, (key, value) => {
            ...
            setAnalysisDebug(safeDebug);
            console.log('🔍 AI Analysis Debug Info:', safeDebug);
          } catch (debugErr) {
            console.warn('Could not log debug info');
        ...
        try {
          ...
          console.error('Error setting analysis:', analysisErr);
          setAnalysis(analysis);
        }
      } catch (aiErr) {
                  ...
                  <div className={`text-xs px-3 py-2 rounded mb-4 ${
                    ...
                    {analysisDebug?.isQuotaError && (
                      <div className="mt-2 text-xs text-red-700 space-y-1">
                        <div><strong>🚨 API Quota Exceeded:</strong></div>
                        <div>• Free tier limit: 20 requests per day</div>
                        {analysisDebug.retryDelay && (
                          <div>• Rate limit: Retry after {analysisDebug.retryDelay} seconds</div>
                        )}
                        <div>• Daily limit: Resets in 24 hours</div>
                        <div className="mt-2 pt-2 border-t border-red-300">
                          <strong>Solutions:</strong>
                          <ul className="list-disc list-inside mt-1 space-y-0.5">
                            <li>Wait 24 hours for daily limit to reset</li>
                            <li>Upgrade to paid plan: <a href="https://ai.google.dev/pricing" target="_blank" rel="noopener noreferrer" className="underline">ai.google.dev/pricing</a></li>
                            <li>Use a different API key</li>
                          </ul>
                        </div>
                      </div>
                    )}
                    {analysisSource !== 'gemini' && !analysisDebug?.isQuotaError && (
